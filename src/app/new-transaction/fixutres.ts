import routes from '../routes';

export const tabsMenu = [
  { id: 1, title: 'Send tokens', url: routes.newTransactionSendToken },
  { id: 2, title: 'Transaction builder', url: routes.newTransactionTrxBuilder },
];

export enum TYPE_ABI {
  PROXY = 'PROXY',
  IMPLEMENTATION = 'IMPLEMENTATION',
}

// TODO
// SEPOLIA - 0x1A764e4738bc9fA91c2b8368caFe099304a6B27a
export const TEST_IMPRLEMENTATION_ABI = [
  { type: 'constructor', inputs: [], stateMutability: 'nonpayable' },
  {
    name: 'AddedOwner',
    type: 'event',
    inputs: [{ name: 'owner', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ApproveHash',
    type: 'event',
    inputs: [
      { name: 'approvedHash', type: 'bytes32', indexed: true, internalType: 'bytes32' },
      { name: 'owner', type: 'address', indexed: true, internalType: 'address' },
    ],
    anonymous: false,
  },
  {
    name: 'ChangedFallbackHandler',
    type: 'event',
    inputs: [{ name: 'handler', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ChangedGuard',
    type: 'event',
    inputs: [{ name: 'guard', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ChangedThreshold',
    type: 'event',
    inputs: [{ name: 'threshold', type: 'uint256', indexed: false, internalType: 'uint256' }],
    anonymous: false,
  },
  {
    name: 'DisabledModule',
    type: 'event',
    inputs: [{ name: 'module', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'EnabledModule',
    type: 'event',
    inputs: [{ name: 'module', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ExecutionFailure',
    type: 'event',
    inputs: [
      { name: 'txHash', type: 'bytes32', indexed: false, internalType: 'bytes32' },
      { name: 'payment', type: 'uint256', indexed: false, internalType: 'uint256' },
    ],
    anonymous: false,
  },
  {
    name: 'ExecutionFromModuleFailure',
    type: 'event',
    inputs: [{ name: 'module', type: 'address', indexed: true, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ExecutionFromModuleSuccess',
    type: 'event',
    inputs: [{ name: 'module', type: 'address', indexed: true, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'ExecutionSuccess',
    type: 'event',
    inputs: [
      { name: 'txHash', type: 'bytes32', indexed: false, internalType: 'bytes32' },
      { name: 'payment', type: 'uint256', indexed: false, internalType: 'uint256' },
    ],
    anonymous: false,
  },
  {
    name: 'RemovedOwner',
    type: 'event',
    inputs: [{ name: 'owner', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false,
  },
  {
    name: 'SafeReceived',
    type: 'event',
    inputs: [
      { name: 'sender', type: 'address', indexed: true, internalType: 'address' },
      { name: 'value', type: 'uint256', indexed: false, internalType: 'uint256' },
    ],
    anonymous: false,
  },
  {
    name: 'SafeSetup',
    type: 'event',
    inputs: [
      { name: 'initiator', type: 'address', indexed: true, internalType: 'address' },
      { name: 'owners', type: 'address[]', indexed: false, internalType: 'address[]' },
      { name: 'threshold', type: 'uint256', indexed: false, internalType: 'uint256' },
      { name: 'initializer', type: 'address', indexed: false, internalType: 'address' },
      { name: 'fallbackHandler', type: 'address', indexed: false, internalType: 'address' },
    ],
    anonymous: false,
  },
  {
    name: 'SignMsg',
    type: 'event',
    inputs: [{ name: 'msgHash', type: 'bytes32', indexed: true, internalType: 'bytes32' }],
    anonymous: false,
  },
  { type: 'fallback', stateMutability: 'nonpayable' },
  {
    name: 'VERSION',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'string', internalType: 'string' }],
    stateMutability: 'view',
  },
  {
    name: 'addOwnerWithThreshold',
    type: 'function',
    inputs: [
      { name: 'owner', type: 'address', internalType: 'address' },
      { name: '_threshold', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'approveHash',
    type: 'function',
    inputs: [{ name: 'hashToApprove', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'approvedHashes',
    type: 'function',
    inputs: [
      { name: '', type: 'address', internalType: 'address' },
      { name: '', type: 'bytes32', internalType: 'bytes32' },
    ],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view',
  },
  {
    name: 'changeThreshold',
    type: 'function',
    inputs: [{ name: '_threshold', type: 'uint256', internalType: 'uint256' }],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'checkNSignatures',
    type: 'function',
    inputs: [
      { name: 'dataHash', type: 'bytes32', internalType: 'bytes32' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'signatures', type: 'bytes', internalType: 'bytes' },
      { name: 'requiredSignatures', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [],
    stateMutability: 'view',
  },
  {
    name: 'checkSignatures',
    type: 'function',
    inputs: [
      { name: 'dataHash', type: 'bytes32', internalType: 'bytes32' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'signatures', type: 'bytes', internalType: 'bytes' },
    ],
    outputs: [],
    stateMutability: 'view',
  },
  {
    name: 'disableModule',
    type: 'function',
    inputs: [
      { name: 'prevModule', type: 'address', internalType: 'address' },
      { name: 'module', type: 'address', internalType: 'address' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'domainSeparator',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'bytes32', internalType: 'bytes32' }],
    stateMutability: 'view',
  },
  {
    name: 'enableModule',
    type: 'function',
    inputs: [{ name: 'module', type: 'address', internalType: 'address' }],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'encodeTransactionData',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
      { name: 'safeTxGas', type: 'uint256', internalType: 'uint256' },
      { name: 'baseGas', type: 'uint256', internalType: 'uint256' },
      { name: 'gasPrice', type: 'uint256', internalType: 'uint256' },
      { name: 'gasToken', type: 'address', internalType: 'address' },
      { name: 'refundReceiver', type: 'address', internalType: 'address' },
      { name: '_nonce', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [{ name: '', type: 'bytes', internalType: 'bytes' }],
    stateMutability: 'view',
  },
  {
    name: 'execTransaction',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
      { name: 'safeTxGas', type: 'uint256', internalType: 'uint256' },
      { name: 'baseGas', type: 'uint256', internalType: 'uint256' },
      { name: 'gasPrice', type: 'uint256', internalType: 'uint256' },
      { name: 'gasToken', type: 'address', internalType: 'address' },
      { name: 'refundReceiver', type: 'address', internalType: 'address payable' },
      { name: 'signatures', type: 'bytes', internalType: 'bytes' },
    ],
    outputs: [{ name: 'success', type: 'bool', internalType: 'bool' }],
    stateMutability: 'payable',
  },
  {
    name: 'execTransactionFromModule',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
    ],
    outputs: [{ name: 'success', type: 'bool', internalType: 'bool' }],
    stateMutability: 'nonpayable',
  },
  {
    name: 'execTransactionFromModuleReturnData',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
    ],
    outputs: [
      { name: 'success', type: 'bool', internalType: 'bool' },
      { name: 'returnData', type: 'bytes', internalType: 'bytes' },
    ],
    stateMutability: 'nonpayable',
  },
  {
    name: 'getChainId',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view',
  },
  {
    name: 'getModulesPaginated',
    type: 'function',
    inputs: [
      { name: 'start', type: 'address', internalType: 'address' },
      { name: 'pageSize', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [
      { name: 'array', type: 'address[]', internalType: 'address[]' },
      { name: 'next', type: 'address', internalType: 'address' },
    ],
    stateMutability: 'view',
  },
  {
    name: 'getOwners',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'address[]', internalType: 'address[]' }],
    stateMutability: 'view',
  },
  {
    name: 'getStorageAt',
    type: 'function',
    inputs: [
      { name: 'offset', type: 'uint256', internalType: 'uint256' },
      { name: 'length', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [{ name: '', type: 'bytes', internalType: 'bytes' }],
    stateMutability: 'view',
  },
  {
    name: 'getThreshold',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view',
  },
  {
    name: 'getTransactionHash',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
      { name: 'safeTxGas', type: 'uint256', internalType: 'uint256' },
      { name: 'baseGas', type: 'uint256', internalType: 'uint256' },
      { name: 'gasPrice', type: 'uint256', internalType: 'uint256' },
      { name: 'gasToken', type: 'address', internalType: 'address' },
      { name: 'refundReceiver', type: 'address', internalType: 'address' },
      { name: '_nonce', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [{ name: '', type: 'bytes32', internalType: 'bytes32' }],
    stateMutability: 'view',
  },
  {
    name: 'isModuleEnabled',
    type: 'function',
    inputs: [{ name: 'module', type: 'address', internalType: 'address' }],
    outputs: [{ name: '', type: 'bool', internalType: 'bool' }],
    stateMutability: 'view',
  },
  {
    name: 'isOwner',
    type: 'function',
    inputs: [{ name: 'owner', type: 'address', internalType: 'address' }],
    outputs: [{ name: '', type: 'bool', internalType: 'bool' }],
    stateMutability: 'view',
  },
  {
    name: 'nonce',
    type: 'function',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view',
  },
  {
    name: 'removeOwner',
    type: 'function',
    inputs: [
      { name: 'prevOwner', type: 'address', internalType: 'address' },
      { name: 'owner', type: 'address', internalType: 'address' },
      { name: '_threshold', type: 'uint256', internalType: 'uint256' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'requiredTxGas',
    type: 'function',
    inputs: [
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'value', type: 'uint256', internalType: 'uint256' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'operation', type: 'uint8', internalType: 'enum Enum.Operation' },
    ],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'nonpayable',
  },
  {
    name: 'setFallbackHandler',
    type: 'function',
    inputs: [{ name: 'handler', type: 'address', internalType: 'address' }],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'setGuard',
    type: 'function',
    inputs: [{ name: 'guard', type: 'address', internalType: 'address' }],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'setup',
    type: 'function',
    inputs: [
      { name: '_owners', type: 'address[]', internalType: 'address[]' },
      { name: '_threshold', type: 'uint256', internalType: 'uint256' },
      { name: 'to', type: 'address', internalType: 'address' },
      { name: 'data', type: 'bytes', internalType: 'bytes' },
      { name: 'fallbackHandler', type: 'address', internalType: 'address' },
      { name: 'paymentToken', type: 'address', internalType: 'address' },
      { name: 'payment', type: 'uint256', internalType: 'uint256' },
      { name: 'paymentReceiver', type: 'address', internalType: 'address payable' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'signedMessages',
    type: 'function',
    inputs: [{ name: '', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view',
  },
  {
    name: 'simulateAndRevert',
    type: 'function',
    inputs: [
      { name: 'targetContract', type: 'address', internalType: 'address' },
      { name: 'calldataPayload', type: 'bytes', internalType: 'bytes' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  {
    name: 'swapOwner',
    type: 'function',
    inputs: [
      { name: 'prevOwner', type: 'address', internalType: 'address' },
      { name: 'oldOwner', type: 'address', internalType: 'address' },
      { name: 'newOwner', type: 'address', internalType: 'address' },
    ],
    outputs: [],
    stateMutability: 'nonpayable',
  },
  { type: 'receive', stateMutability: 'payable' },
];
