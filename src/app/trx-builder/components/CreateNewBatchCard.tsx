...
const StyledDragAndDropFileContainer = styled.div<{
  ...
  border: 2px dashed ${({ theme, error }) => (error ? theme.palettes.error : '#008c73')};
  border-radius: 8px;
  background-color: ${({ theme, error }) => (error ? alpha(theme.palettes.error, 0.7) : '#eaf7f4')};
  padding: 24px;
  ...
  ${({ dragOver, error, theme }) => {
    ...
    return `
      border-color: ${error ? theme.palettes.error : '#008c73'};
      background-color: ${error ? alpha(theme.palettes.error, 0.7) : '#eaf7f4'};
    `;
  }}
`;
