...
import {
  assertValidSafeVersion,
  createEthersAdapter,
  createReadOnlyEthersAdapter,
...
export const isValidMasterCopy = (
...
const getGnosisSafeContractEthers = async (
  safe: SafeInfo,
...
export const _getMinimumMultiSendCallOnlyVersion = (safeVersion: SafeInfo['version']) => {
  ...
  return semver.gte(safeVersion, INITIAL_CALL_ONLY_VERSION)
    ? safeVersion
    : INITIAL_CALL_ONLY_VERSION;
};
...
export const getReadOnlyProxyFactoryContract = (
  chainId: string,
  safeVersion: SafeInfo['version']
) => {
  const ethAdapter = createReadOnlyEthersAdapter();
