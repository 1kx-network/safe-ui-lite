...
export const SOCIAL_WALLET_OPTIONS: any = (() => {
  const SOCIAL_WALLET_OPTIONS_PRODUCTION =
    process.env.NEXT_PUBLIC_SOCIAL_WALLET_OPTIONS_PRODUCTION || '{}';
  const SOCIAL_WALLET_OPTIONS_STAGING =
    process.env.NEXT_PUBLIC_SOCIAL_WALLET_OPTIONS_STAGING || '{}';

  try {
    return JSON.parse(
      IS_PRODUCTION ? SOCIAL_WALLET_OPTIONS_PRODUCTION : SOCIAL_WALLET_OPTIONS_STAGING
    );
  } catch (error) {
