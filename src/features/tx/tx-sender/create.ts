...
export const createAddOwnerTx = async (
  ...
  const data = contract.encode('addOwnerWithThreshold', [
    txParams.ownerAddress,
    txParams.threshold,
  ]);
...
export const createSwapOwnerTx = async (
  ...
  const contract = await getReadOnlyGnosisSafeContract(chain, safeVersion);
  const data = contract.encode('swapOwner', [
    SENTINEL_ADDRESS,
    txParams.oldOwnerAddress,
    txParams.newOwnerAddress,
  ]);
