...
export const dispatchOnChainSigning = async (
  ...
  const options =
    chainId === chains.zksync ? { gasLimit: ZK_SYNC_ON_CHAIN_SIGNATURE_GAS_LIMIT } : undefined;
...
export const dispatchBatchExecution = async (
  ...
  const txData = multiSendContract.encode('multiSend', [multiSendTxData]);
  const wallet = await assertWalletChain(onboard, chainId);
...
export const dispatchTxRelay = async (
  ...
  const transactionToRelay = safeTx;
...
export const dispatchBatchExecutionRelay = async (
  ...
  const data = multiSendContract.contract.interface.encodeFunctionData('multiSend', [
    multiSendTxData,
  ]);
  const groupKey = multiSendTxData;
